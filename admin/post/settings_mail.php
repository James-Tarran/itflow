...
if (isset($_POST['oauth_connect_microsoft_mail'])) {
    ...
    if ($config_imap_provider !== 'microsoft_oauth' && !in_array($config_smtp_provider, ['microsoft_oauth', 'microsoft_graph'], true)) {
        flashAlert("Please set the SMTP or IMAP Provider to a Microsoft 365 option and save it before connecting.", 'error');
    ...
    if ($needs_graph) {
        ...
        flashAlert("All required Microsoft 365 resources are already connected. If sending/receiving still fails, check mailbox-level permissions (e.g. Send As) rather than reconnecting.");
        redirect($mail_tab_redirect);
    } else {
        flashAlert("No Microsoft 365 Sending or Receiving provider is configured.", 'error');
        redirect($mail_tab_redirect);
    }
